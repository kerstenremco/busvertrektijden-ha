...
from .const import DOMAIN, CONF_STOP_NAME, CONF_SHORT_NAME_FILTER, CONF_TRIP_HEADSIGN_FILTER, API_URL
...
class BusVertrektijdenConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    def __init__(self):
        ...
        self._trip_headsign_filter: str | None = None
    ...
    async def _get_stops(self):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{API_URL}/stops?q={self._search_query}") as response:
    ...
    async def async_step_filters(self, user_input=None):
        """Optional step: user sets filters for short name and trip headsign."""
        if user_input is None:
            return self.async_show_form(
                step_id="filters",
                data_schema=vol.Schema({
                    vol.Optional("short_name_filter"): str,
                    vol.Optional("trip_headsign_filter"): str,
                })
            )

        self._short_name_filter = normalize_commas(
            user_input.get("short_name_filter"))
        self._trip_headsign_filter = normalize_commas(
            user_input.get("trip_headsign_filter"))
        return self.async_create_entry(
            title=self._selected_stop,
            data={CONF_STOP_NAME: self._selected_stop, CONF_SHORT_NAME_FILTER: self._short_name_filter,
                  CONF_TRIP_HEADSIGN_FILTER: self._trip_headsign_filter},
        )
